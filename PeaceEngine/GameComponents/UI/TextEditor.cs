    ...
    public class TextEditor : Control
    {
        ...
        protected override void OnKeyEvent(KeyboardEventArgs e)
        {
            ...
            else if (e.Key == Keys.End)
            {
                ...
                {
                    _currentIndex = MathHelper.Clamp(_currentIndex + current.Substring(_currentChar).Length, 0, _textRaw.Length);
                    recalculateLines();
        ...
        protected override void OnUpdate(GameTime time)
        {
            if (_font == null)
            {
                _font = Theme.GetFont(Themes.TextStyle.Regular);
                recalculateLines();
            }
            if (_lastWidth != Width)
            {
        ...
        {
            Theme.DrawTextEditorBackground(gfx);
            int lineHeight = (int)_font.MeasureString("#").Y;
            if (string.IsNullOrEmpty(_textRaw))
            {
                if (HasFocused && _caretVisible)
                    gfx.FillRectangle(2, 2, 1, lineHeight, Theme.GetTextColor(Themes.TextStyle.Regular));
            }
            else
            {
                for (int i = 0; i < _lines.Length; i++)
                {
                    ...
                    {
                        gfx.DrawString(line, 2, 2 + (lineHeight * i), Theme.GetTextColor(Themes.TextStyle.Regular), _font, TextAlignment.Left, 0, TextRenderers.WrapMode.None);
                        if (_caretVisible)
                        {
                            string toCaret = (_currentChar == 0) ? "" : line.Substring(0, _currentChar);
                            int caretX = (int)_font.MeasureString(toCaret).X;
                            gfx.FillRectangle(2 + caretX, 2 + (lineHeight * i), 1, lineHeight, Theme.GetTextColor(Themes.TextStyle.Regular));
                        }
                    }
                    else
                    {
                        gfx.DrawString(line, 2, 2 + (lineHeight * i), Theme.GetTextColor(Themes.TextStyle.Regular) * 0.5f, _font, TextAlignment.Left, 0, TextRenderers.WrapMode.None);
                    }
                }
            }
        }
