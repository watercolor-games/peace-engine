...
namespace Plex.Engine.GraphicsSubsystem
{
    ...
    public class PrimitiveBatcher
    {
        ...
        public PrimitiveBatcher(IRenderer renderer)
        {
            Renderer = renderer;
        ...
        public void SetSprite(Texture2D texture, Rectangle srcRect)
        {
        ...
        {
            FillTriangleStrip(EnumerableExtensions.Yield(v1, v2, v3).Select(v => CreateVertex(v, Vector2.Zero, c)));
        }
        ...
        public void FillRoundedRect(RectangleF rectangle, float radius, int segments, Color color, RectangleF? uvRect = null)
        {
            ...
            FillRect(innerRect, color, MathHelpers.LinearMap(innerRect, outerRect, ur));
            ...
            FillRect(leftRect, color, MathHelpers.LinearMap(leftRect, outerRect, ur)); // left
            FillRect(rightRect, color, MathHelpers.LinearMap(rightRect, outerRect, ur)); // right
            FillRect(topRect, color, MathHelpers.LinearMap(topRect, outerRect, ur)); // top
            FillRect(bottomRect, color, MathHelpers.LinearMap(bottomRect, outerRect, ur)); // top
            ...
            FillCircleSegment(tl, radius, LeftAngle, TopAngle, color, segments, MathHelpers.LinearMap(tlRect, outerRect, ur));
            FillCircleSegment(tr, radius, TopAngle, RightEndAngle, color, segments, MathHelpers.LinearMap(trRect, outerRect, ur));
            FillCircleSegment(br, radius, RightStartAngle, BotAngle, color, segments, MathHelpers.LinearMap(brRect, outerRect, ur));
            FillCircleSegment(bl, radius, BotAngle, LeftAngle, color, segments, MathHelpers.LinearMap(blRect, outerRect, ur));
        }
        ...
        public void FillCircleSegment(Vector2 center, float radius, float start, float end, Color color, int sides, RectangleF? uvRect = null)
        {
            ...
            var vs = ps.Select(p => CreateVertex(p, MathHelpers.LinearMap(p, fromRect, toRect), color));
            var vCenter = CreateVertex(center, MathHelpers.LinearMap(center, fromRect, toRect), color);
        ...
        private Vertex CreateVertex(Vector2 p, Vector2 uv, Color c)
        {
            ...
            var actualUv = MathHelpers.LinearMap(uv, RectangleF.Unit, _spriteUv);
            if (_useMatrix)
                p = Vector2.Transform(p, TransformMatrix.ToMonoGame());
            var v3 = new Vector3(p.X, p.Y, 0);
            return new Vertex(v3, c, actualUv);
        }
        ...
        {
            var newPos = Vector3.Transform(vertex.Position, transformMatrix.ToMonoGame());
            vertex = new Vertex(newPos, vertex.Color, vertex.TextureCoordinate);
        }
