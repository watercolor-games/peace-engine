...
namespace Plex.Engine.GUI
{
    public class ListBox : Control
    {
        ...
        {
            int itemHeight = (_itemPad * 2) + (int)Theme.GetFont(Themes.TextFontStyle.System).MeasureString("#").Y;
        ...
        protected override void OnPaint(GameTime time, GraphicsContext gfx)
        {
            ...
            for (int i = 0; i < Items.Count; i++)
            {
                ...
                var foreground = Theme.GetFontColor(Themes.TextFontStyle.System);
                var accent = Theme.GetAccentColor();

                if(selected)
                {
                    gfx.FillRectangle(new RectangleF(0, y, Width, itemHeight), accent);
                }
                else if(hovered)
                {
                    gfx.FillRectangle(new RectangleF(0, y, Width, itemHeight), accent * 0.5F);
                }

                gfx.DrawString(Theme.GetFont(Themes.TextFontStyle.System), itemText, new Vector2(x, y + _itemPad), foreground);
            }
        }
